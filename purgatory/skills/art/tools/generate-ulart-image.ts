...
OPTIONS:
  ...
  --creative-variations <n>  Generate N variations (appends -v1, -v2, etc. to output filename)
                             Use with Qara's be-creative skill for true prompt diversity
...
NOTE: For true creative diversity with different prompts, use the creative workflow in Qara which
...
async function main(): Promise<void> {
  try {
    ...
    if (args.creativeVariations && args.creativeVariations > 1) {
      ...
      console.log(`   For true creative diversity, use the creative workflow in Qara with be-creative skill\n`);
