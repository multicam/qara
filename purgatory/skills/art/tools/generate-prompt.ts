...
type CharacterFocus = "maya" | "qara" | "both";
...
const CHARACTER_DESCRIPTIONS = {
  ...
  qara: `Qara is a young boy with a slightly oval head, a soft messy hair tuft on top (his signature feature), and NO glasses. He wears a simple t-shirt and shorts or pants. He has a stick-figure body with thin limbs and a slightly oversized head, with minimal facial features.`,
  both: `Two recurring child characters: Maya and Qara. Maya is a young, highly curious girl with a round head, simple short hair, and big round glasses. Qara is a young boy with a slightly oval head, a soft messy hair tuft, and a simple t-shirt and shorts or pants. Both have stick-figure bodies with thin limbs and slightly oversized heads, with minimal facial features.`,
};
...
function selectCharacter(
  ...
  if (compositionType === "workshop") return "qara";
...
function buildVisualMetaphor(
  ...
  switch (compositionType) {
    case "observation":
      metaphor = `${characterFocus === "maya" ? "Maya" : "Qara"} positioned in the left quarter of the frame, small and observing with ${analysis.tone}, looking at a large visual element on the right that represents the core concept of ${analysis.theme}`;
      break;
    case "horizon":
      metaphor = `${characterFocus === "both" ? "Maya and Qara" : characterFocus === "maya" ? "Maya" : "Qara"} in the foreground, facing a wide distant horizon filled with tiny elements representing future possibilities related to ${analysis.theme}`;
      break;
    case "dialogue":
      metaphor = `Maya and Qara positioned with space between them, interacting with a shared element or concept in the center, representing different perspectives on ${analysis.theme}`;
      break;
    case "workshop":
      metaphor = `${characterFocus === "qara" ? "Qara" : "Both Maya and Qara"} actively building or creating, with elements spreading horizontally showing the process of making something related to ${analysis.theme}`;
      break;
    case "aura":
      metaphor = `${characterFocus === "maya" ? "Maya" : characterFocus === "qara" ? "Qara" : "The character"} surrounded by a soft, translucent aura bubble containing tiny symbolic icons representing aspects of ${analysis.theme}`;
      break;
  }
...
Show ${characterFocus === "both" ? "Maya and Qara" : characterFocus} in a ${compositionType} scene${imageType === "blog-header" ? " optimized for horizontal 16:9 composition" : ""}. ${emotionalToneDescription.charAt(0).toUpperCase() + emotionalToneDescription.slice(1)}, interacting with ${coreObjectDescription}.
...
function main() {
  ...
  if (!args.input) {
    ...
    console.error("  --character      maya | qara | both");
