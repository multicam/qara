...
**Purpose:** Automated documentation of ALL work performed by Qara and specialized agents
...
The Universal Output Capture System (UOCS) is Qara's automatic memory - capturing every feature, bug fix, decision, learning, research, and session through hooks with **zero manual effort**.
...
### 2. Stop Hook
**Triggers:** Main agent (Qara) task completion
...
**Implementation:** `~/.claude/hooks/initialize-qara-session.ts`
...
# Context
project: dashboard
executor: qara|architect|engineer|designer|researcher
session_id: uuid
...
- ✅ SessionStart (initialize-qara-session.ts)
