...
**Design sub-skills (post 2026-04-16 consolidation, 16 activation surfaces):** **Upstream symlinks (12):** impeccable (consolidates frontend-design + teach-impeccable + extract; subcommands: craft/teach/extract), shape, layout (was arrange), polish (absorbs normalize + onboard + state phrases 2026-04-16), adapt, animate, audit, clarify, critique, harden, optimize, visual-explainer. **Local skills (4, added 2026-04-16):** `tune` (merges bolder+quieter+colorize dispatcher), `impeccable-typeset` (local wrapper over upstream typeset), `tokens` (thin alias for `/impeccable extract`), `flows` (product-scoped user journey + IA). **Pruned as redundant with impeccable:** extract (standalone), delight, distill, overdrive.
