...
Show format examples inline (e.g., helper text below the label), not as standalone instructions. Placeholders can hint at format but must not be the only source of that information — they disappear on input. For non-obvious fields, explain why you're asking.
