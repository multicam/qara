...
8pt systems are too coarse—you'll frequently need 12px (between 8 and 16). Use 4pt for granularity: 0.25rem, 0.5rem, 0.75rem, 1rem, 1.5rem, 2rem, 3rem, 4rem, 6rem. Use `rem` for spacing tokens (scales with user font-size). Use `px` only for breakpoints, touch targets, and 1-2px optical adjustments.
...
Buttons can look small but need large touch targets (44px minimum — px is correct here, it's a physical dimension). Use padding or pseudo-elements:
...
**Avoid**: Arbitrary spacing values outside your scale. Making all spacing equal (variety creates hierarchy). Creating hierarchy through size alone - combine size, weight, color, and space. Hard-coded `px` values for spacing — use rem-based design tokens (`--space-sm`, `--space-md`, etc.).
