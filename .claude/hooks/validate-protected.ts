 ...
 *
 * Ensures PAI-specific files haven't been overwritten with Qara content.
...
function checkFileContent(filePath: string, manifest: ProtectedManifest): {
    ...
    for (const [categoryName, category] of Object.entries(manifest.protected)) {
        ...
        // Must not contain private Qara data (skip if exception file)
        if (category.validation.includes('private Qara data') && !isException) {
            const privatePatterns = [
                /\/Users\/daniel\/\.claude\/skills\/personal/,
                /daemon\.plist/,
                /Qara \(Personal AI Infrastructure\)/,
            ];

            for (const pattern of privatePatterns) {
                if (pattern.test(content)) {
                    violations.push(`Contains private Qara reference: ${pattern.source}`);
                }
            }
        }
...
async function main() {
    ...
    if (hasViolations) {
        ...
        console.log('  3. Remove references to private Qara data');
        console.log('  4. Ensure PAI-specific files reference "PAI" not "Qara"');
